...
#
# bash has no autoload: 'complete -F' has to be registered at startup, so its
# completion is sourced eagerly and costs a parse of ~15 KB in every shell.
# zsh does have one, and using it is the whole point of generating '_jlo' into
# a directory of its own: put the *directory* on $fpath and zsh reads the file
# on the first Tab press, not on every shell start. Sourcing it here instead
# would undo the reason completions are generated at install time (see the
# comment above): the subprocess was avoided, the parse was not.
